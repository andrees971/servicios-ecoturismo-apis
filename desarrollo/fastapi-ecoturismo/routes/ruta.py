...
def create_ruta(rut: Ruta):
    new_ruta = {"ruta_id": rut.ruta_id, "nombre": rut.nombre, "distancia": rut.distancia, "riesgo": rut.riesgo, "descripcion": rut.descripcion, "ubicacion": rut.ubicacion, "fecha": rut.fecha, "empresa_id": rut.empresa_id, "seguro_id": rut.seguro_id}
...
@rut.get("/ruta/{id}", response_model=Ruta, tags=["ruta"])
def get_ruta_id(id: str):
...
def update_ruta(id: str, rut: Ruta):
    conn.execute(ruta.update().values(nombre = rut.nombre, distancia = rut.distancia, riesgo = rut.riesgo, descripcion = rut.descripcion, ubicacion = rut.ubicacion, fecha = rut.fecha, empresa_id = rut.empresa_id, seguro_id = rut.seguro_id).where(ruta.c.ruta_id == id))
    return conn.execute(ruta.select().where(ruta.c.ruta_id == id)).first()
